...
<div class="app">
    <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            $(Get-NavItem -href '/admin/settings' -label 'Paramètres' -icon $icons.Settings -currentPath $path)
        </nav>
        ...
        while ($http.IsListening) {
            ...
            try {
                ...
                $result = switch ($path) {
                    ...
                    "/admin/settings" { Handle-AdminSettings -method $method -postParams $postParams -session $session -sessionToken $sessionCookie.Value }
                    default          { Handle-Default -baseUrl (Get-AppSetting 'PUB_URL' $baseUrl) }
                }
