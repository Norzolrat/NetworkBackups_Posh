...
- Après connexion, l'utilisateur arrive sur `/conf` (page par défaut). La section Administration de la barre latérale donne accès à :
  ...
  - `/admin/settings` : **paramètres** — changement du mot de passe admin (vérification de l'actuel, re-haché en Argon2id, les autres sessions sont déconnectées), **planification des sauvegardes** (activation/désactivation du cron, fréquences prédéfinies ou expression cron libre — appliquée immédiatement et restaurée au démarrage du conteneur), **sauvegarde distante optionnelle** (FTP ou SMB : une archive `tar.gz` des configurations est envoyée après chaque sauvegarde, avec rétention configurable — les archives les plus anciennes au-delà du nombre conservé sont supprimées à chaque run ; identifiants stockés en `SecureString` dans `/app/secrets/remote.xml`, bouton « Tester la connexion ») et édition des variables (`ADMIN_USER`, `PUB_URL`, `WEB_PREFIX`, `WEB_ADDR`, `WEB_PORT`). Les valeurs définies ici sont stockées dans `/app/secrets/settings.json` (600) et **priment sur les variables d'environnement** ; un champ vide revient à la variable d'environnement. Les variables d'écoute web (↻) sont prises en compte au redémarrage du conteneur.
